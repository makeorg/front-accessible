...
describe('SequenceFooterComponent', () => {
  it('Check a11y rules', () => {
    ...
    expect(wrapper.find(FooterNavStyle).prop('aria-labelledby')).toBe(
      'footer_title'
    );
    expect(wrapper.find(FooterNavStyle).text()).toBe(
      'footer_sequence.see_morefoofooter_sequence.link'
    );
    expect(wrapper.find(FooterTitleStyle).prop('color')).toBe(
      props.questionConfiguration.theme.footerFontColor
    );
    expect(wrapper.find(FooterLinkStyle).prop('color')).toBe(
      props.questionConfiguration.theme.footerFontColor
    );
    expect(wrapper.find(FooterLinkStyle).prop('href')).toBe(
      props.questionConfiguration.consultationUrl
    );
  });
});
