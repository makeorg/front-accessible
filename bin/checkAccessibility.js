...
const dataRelaticePath = 'shared/services/accessibilityScoring.yaml';
const docRelativePath = 'docs/accessibility.md';
const constantRelativePath = 'shared/constants/accessibilitySummary.js';
const configuration = {
  dataFile: path.resolve(process.cwd(), dataRelaticePath),
  documentationFile: path.resolve(process.cwd(), docRelativePath),
  constantFile: path.resolve(process.cwd(), constantRelativePath),
  regReplacement: /<auto-generated-accessibility-doc>([\s\S]*?)<\/auto-generated-accessibility-doc>/g,
  rgaaGuideUrl:
...
Edit file \`${dataRelaticePath}\` and generate this documentation.
...
(async () => {
  ...
  try {
    ...
    const detailData = accessibilityTestData.map(item => {
      ...
      const criteriaSuccess = criteriaScoring.find(i => i.criteria === criteria)
        .success;
